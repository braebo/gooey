...
/**
 * Detects the current operating system.  On the server, the runtime is returned instead.
...
export function getOS(request?: Request): UserAgent {
	...
	if (platform.toLowerCase().startsWith('node')) return 'node'
	if (platform.toLowerCase().startsWith('deno')) return 'deno'
