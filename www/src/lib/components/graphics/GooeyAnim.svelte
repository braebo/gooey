<script lang="ts">
	// import type { WindowInstance } from '../../../../../dist/build/shared/WindowManager'

	import { Gooey, Color, type InputNumber } from 'gooey'
	...
	let turbulence: SVGFETurbulenceElement
	let displacement: SVGFEDisplacementMapElement

	let slider = $state<SVGRectElement>()
	let thumb = $state<SVGRectElement>()
	let sliderInput: InputNumber

	let orb1: SVGEllipseElement
	let orb2: SVGEllipseElement
	let orb3: SVGEllipseElement
	let orb4: SVGEllipseElement
	...
	let p = $state({
		...
		orbs: {
			...
			orb3: {
				...
				cy: 55,
				radius: 8.5,
				colorA: new Color('#0095ff'), // todo - the fucking Color class is busted (hsl string parser is wrong)
	...
	// let progress = $derived(mapRange(p.slider, 0, 1, 0, viewBox.width))
	let progress = $derived.by(() => {
		if (slider === undefined || thumb === undefined) return 1
		const x = +slider.getAttribute('x')!
		const width = +slider.getAttribute('width')!
		const thumbWidth = +thumb.getAttribute('width')!
	...
	onMount(() => {
		...
		gui.bindMany(p, {
			...
			goo: {
				...
				viscosity: { min: 0.004, max: 0.5, step: 0.0001 },
				density: { min: 1, max: 10, step: 1 }
			},
			orbs: {
				size: { min: 0.1, max: 10, step: 0.01 },
	...
	function onpointerdown(e: PointerEvent) {
		grabbing = true
		mouseOffset = e.clientX - thumb!.getBoundingClientRect().left
	}
	...
	const lfoAmount = 0.1
	let lfo = $derived(((0.75 + Math.sin(time * p.speed * 0.0001) * 0.5) * 0.8 + 0.1) * lfoAmount)
...
<!-- <div style="position:absolute;inset:0;margin: auto; width: 10rem;">{lfo.toFixed(2)}</div> -->
<div class="logo">
	<!-- prettier-ignore -->
	<svg
		id="gooey_anim"
	...
	>
		<ellipse id="orb2" bind:this={orb2} cx={p.orbs.orb2.cx} cy={p.orbs.orb2.cy} rx={p.orbs.orb2.radius * p.orbs.size} ry={p.orbs.orb2.radius * p.orbs.size} fill="url(#gooey_anim_gradient_2)" />
		<path
			id="text"
			fill="currentColor"
		...
		/>
		<ellipse id="orb1" bind:this={orb1} cx={p.orbs.orb1.cx} cy={p.orbs.orb1.cy} rx={p.orbs.orb1.radius * p.orbs.size} ry={p.orbs.orb1.radius * p.orbs.size} fill="url(#gooey_anim_gradient_1)" />
		<ellipse id="orb4" bind:this={orb4} cx={p.orbs.orb4.cx} cy={p.orbs.orb4.cy} rx={p.orbs.orb4.radius * p.orbs.size} ry={p.orbs.orb4.radius * p.orbs.size} fill="url(#gooey_anim_gradient_4)" />
		<ellipse id="orb3" bind:this={orb3} cx={p.orbs.orb3.cx} cy={p.orbs.orb3.cy} rx={p.orbs.orb3.radius * p.orbs.size} ry={p.orbs.orb3.radius * p.orbs.size} fill="url(#gooey_anim_gradient_3)" />

		<g id="slider">
			<rect bind:this={slider} id="track" width="242" height="14.3" x="139" y="129.3" fill="currentColor" class="track" rx="8.2" />
