import type { InputOptions, InputPreset, ValidInput, ValidInputValue } from './inputs/Input';
 ...
 */
export type InferOptions<T> = T extends number ? NumberInputOptions : T extends boolean ? SwitchInputOptions : T extends ColorFormat ? ColorInputOptions : T extends string ? TextInputOptions : T extends ButtonGridArrays ? ButtonGridInputOptions : T extends Array<infer T> ? SelectInputOptions<T> : T extends Option<infer T> ? SelectInputOptions<T> : T extends () => void ? ButtonInputOptions : InputOptions;
/**
 * Resolves any provided value to the corresponding {@link ValidInput} associated with the type.
 */
export type InferInput<TValueType> = TValueType extends number ? InputNumber : TValueType extends boolean ? InputSwitch : TValueType extends ColorFormat ? InputColor : TValueType extends string ? InputText : TValueType extends () => void ? InputButton : TValueType extends ButtonGridArrays ? InputButtonGrid : TValueType extends Array<infer T> ? InputSelect<T> : TValueType extends Option<infer T> ? InputSelect<T> : ValidInput;
/**
...
export type InferTargetOptions<TTarget> = {
    [K in keyof TTarget]?: TTarget[K] extends Array<infer U> ? Partial<SelectInputOptions<U>> : TTarget[K] extends LabeledOption<infer U> ? Partial<SelectInputOptions<LabeledOption<U>>> : TTarget[K] extends Function ? Partial<ButtonInputOptions> : TTarget[K] extends ColorFormat ? Partial<ColorInputOptions> : TTarget[K] extends object ? InferTargetOptions<TTarget[K]> & {
 ...
 * @template TTarget - The target object being used to generate inputs.
 */
export type InferInputs<TTarget> = TTarget extends Record<string, any> ? {
    [K in keyof TTarget]: TTarget[K] extends number ? InputNumber : TTarget[K] extends boolean ? InputSwitch : TTarget[K] extends ColorString | ColorObject ? InputColor : TTarget[K] extends string ? InputText : TTarget[K] extends Array<infer T> ? InputSelect<T> : TTarget[K] extends () => void ? InputButton : TTarget[K] extends object ? InferInputs<TTarget[K]> & {
        folder: Folder;
    } : never;
} : never;
/**
...
export type InferFolders<T> = T extends object ? {
    [K in FolderKeys<T>]: InferFolders<T[K]> & {
        folder: Folder;
    };
} : never;
...
export declare class Folder {
     ...
     * })
     */
    addMany<T extends object, const TOptions extends InferTargetOptions<T> = InferTargetOptions<T>, const TInputs extends InferInputs<T> = InferInputs<T>>(target: T, options?: TOptions & {
        /**
        ...
        folders: InferFolders<T>;
        inputs: TInputs;
    };
    bindMany<const T extends object, const TOptions extends InferTargetOptions<T> = InferTargetOptions<T>, const TInputs extends InferInputs<T> = InferInputs<T>>(target: T, options?: TOptions & {
        /**
         * An array of keys to exclude from a target object when generating inputs.
         */
        exclude?: InferTargetKeys<T>[];
